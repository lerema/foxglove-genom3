...
a|.Data structure
[disc]
 * `struct ::or::Environment::OccupancyGrid` `grid_maps`
 ** `sequence< octet >` `data`
 ** `octet` `width`
 ** `octet` `height`
 ** `double` `resolution`
 ...
 * `enum ::FoxgloveStudio::PortType` `port_type` Genom port type ∈ { `or_sensor_frame`, `or_sensor_imu`, `or_sensor_magnetometer`, `or_pose_estimator_state`, `or_sensor_gps`, `or_state_occupancy_grid` }
